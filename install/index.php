...
define('SOFT_NAME', '懒人diy手机网站管理系统'.SOFT_VERSION);
define('DBCHARSET','UTF8');//编码 分为 UTF8 GBK
//----------------------语言包
$lang = array(
'license'=>'<div class="license"><h1>懒人diy手机网站管理系统使用协议</h1>
                <p>版权所有 (c)2013-2014 保留所有权利。 </p>
				<p>本系统包含以下功能：</p>
				<p>1、RBAC权限管理功能； 便捷地对系统中用户进行权限分配，所以权限分配可以在一个页面分配完成。</p>
				<p>2、新闻发布版块；基本的新闻发布、修改、删除，删除文章，图片也自动删除。  </p>
				<p>3、案例发布版块；网站案例的发布、修改、删除，上传图片，生成两张图片，删除案例，相应的图片也自动删除。 </p>
				<p>4、无极限分类功能； 满足新闻分类等的无极限分类的功能，你可以根据实际情况修改。 </p>
				<p>5、每月自动备份功能；管理员每月个第一次登陆后台，系统会后台自动进行当月数据备份功能。 </p>
				<p>6、备份、还原数据库，打包已备份sql文件备份数据量大时，系统会自动分隔备份成多个sql文件，每个sql文件头部记录了当前sql文件包含了那些表数据。支持其他软件导入的sql文件导入（支持导入>200M的sql文件，目前只测试过200M左右的sql文件，虽然支持但是还是不建议这么做）。</p>
				<p>7、打包已备份sql文件，在线解压zip数据文件考虑到节省磁盘空间，你可以将已备份是sql文件打包成zip压缩文件（对应打包的sql文件会被清除），同时你可以在线解压之间zip打包的文件（zip文件保留）。</p>
				<p>8、邮件发送sql备份，下载数据库备份文件在你配置了系统邮件信息后，你可以轻松将你的备份sql文件打包发送到你指定的邮箱中，如果发送的sql文件较多较大，系统考虑到备份邮箱不支持大附件系统将分成多封邮件发送（压缩前50M一封，压缩后一般在6~7M左右），同时你可以将系统备份的sql文件下载到你本地。</p>
				<p>9、数据优化修复功能你可以轻松优化修复你的mysql数据库。 </p>
				<p>10、缓存清除功能你不用去一个一个目录地清除缓存文件了，只要勾选确定就可以了 </p>
				<p>11、JS\CSS 压缩缓存系统加入了minify压缩js、css缓存，为了和TP的分隔符分开，如果你要将你的js、css加入到minify中，你必须使用竖线“|”将多个文件分隔开。 </p>
</div>',
'agreement_yes' => '我同意，继续安装',
'agreement_no' => '我不同意',
'title_install' => '懒人diy手机网站管理系统'.SOFT_VERSION.' 安装程序',
...
$dirfile_items = array(
    'conf' => array('type' => 'file', 'path' => './App/Conf/mysqlconf.php'),
);
...
if($method == 'show_license') {//声明
	...
	if($submit) {
		...
		if($confstatus['status']==1){
			...
			$db->query("INSERT INTO `{$tablepre}admin` (`id`, `username`, `pwd`, `ip`, `logtime`) VALUES
(1, '".$username."', '".$password."', '127.0.0.1', '".time()."')");
			//新增一个标识文件，用来屏蔽重新安装
...
<meta content="DIYWAP手机网站管理系统" name="Copyright" />
...
?>
			<div class="footer">
			    <p>Powered by 懒人diy© v2.0 2012-2014</p>
				<p>版权所有</p>
			</div>
		</div>
		</div>
...
function runquery($sql) {
	...
	$sql = str_replace("\r", "\n", str_replace('lanrendiy_', $tablepre, $sql));
	$ret = array();
	$num = 0;
	foreach(explode(";\n", trim($sql)) as $query) {
		$ret[$num] = '';
